    ...
    <section className="relative w-full h-[92svh] min-h-[540px] sm:min-h-[640px] md:min-h-[700px] max-h-[960px] flex flex-col items-center justify-start overflow-hidden bg-[#d6a978]">
      ...
      <div className="absolute inset-0 w-full h-full">
        ...
        <div className="block sm:hidden absolute inset-0 w-full h-full">
          <Image
            src={resolveImageUrl(siteImages.hero.mobileBackgroundImage || siteImages.hero.backgroundImage)}
            alt="Upanayana Ceremony & Sangeetha Samarpanam"
            fill
            priority
            sizes="100vw"
            className="object-cover object-bottom brightness-105 contrast-[1.02]"
          />
        </div>
      ...
      <div className="relative z-10 flex flex-col items-center justify-start pt-36 xs:pt-40 sm:pt-20 md:pt-24 px-4 text-center max-w-4xl mx-auto">
        {/* Headline in Deep Royal Crimson & Burgundy */}
        <h1 className="text-[28px] xs:text-3xl sm:text-4xl md:text-5xl lg:text-[46px] font-serif text-[#4a0e17] tracking-tight leading-tight font-medium drop-shadow-[0_1px_2px_rgba(255,255,255,0.6)]">
          Two Beginnings. One Family.
        </h1>

        {/* Flourish Divider in Warm Radiant Gold */}
        <div className="flex items-center justify-center gap-2.5 my-2 xs:my-2.5 sm:my-2.5">
          <div className="w-10 xs:w-12 sm:w-16 h-[1.5px] bg-[#b38e5d]" />
          <svg
            viewBox="0 0 24 16"
            className="w-4 h-3 xs:w-4.5 xs:h-3.5 sm:w-5 sm:h-3.5 text-[#b38e5d] fill-none stroke-current stroke-[1.5] drop-shadow-xs"
          >
        ...
        <p className="max-w-[320px] xs:max-w-md sm:max-w-2xl md:max-w-3xl mx-auto text-[13.5px] xs:text-sm sm:text-[14.5px] md:text-[15px] lg:text-base font-serif text-[#3e1f14] leading-relaxed font-normal drop-shadow-[0_1px_1px_rgba(255,255,255,0.5)]">
